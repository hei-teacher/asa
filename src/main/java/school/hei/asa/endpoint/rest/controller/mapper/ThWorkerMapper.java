...
public class ThWorkerMapper {
  ...
  public List<ThWorkerLevelHistory> toTh(List<WorkerLevelHistory> workerLevelHistories) {
    return range(0, workerLevelHistories.size())
            .mapToObj(i -> {
              ...
              var nextEntrance = (i == 0)
                      ? now()
                      : workerLevelHistories.get(i - 1).entranceInstant();

              var zoneId = ZoneId.of("UTC");

              double totalDaysWorked = missionExecutionPercentageSumByWorker(
                      current.worker(),
                      current.entranceInstant().atZone(zoneId).toLocalDate(),
                      nextEntrance.atZone(zoneId).toLocalDate()
              );

              var contractType = switch (current.contractType()) {
              ...
              var totalWorkDays = contractWithTotalWorkDays.equals(current.contractType())
  ...
  private Double missionExecutionPercentageSumByWorker(
    ...
    return missionExecutionRepository.missionExecutionsByDateBetween(worker, startDate, endDate)
            .stream()
            .filter(jme -> {
