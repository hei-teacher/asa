...
public class DailyExecutionController {
  ...
  public String createDailyExecution(Authentication authentication, ThDailyExecutionForm dmeForm) {
    return createDailyExecution(authentication, dmeForm, new org.springframework.web.servlet.mvc.support.RedirectAttributesModelMap());
  }
  ...
  public String createDailyExecution(
    ...
    if (activeContractOpt.isPresent() && remainingDaysAfter < lowRemainingDaysAlertService.getLowRemainingDaysThreshold()) {
        lowRemainingDaysAlertService.checkAndAlert(worker, activeContractOpt.get(), (long) remainingDaysAfter);
        redirectAttributes.addFlashAttribute("toastMessage", "Attention : Il vous reste " + (long) remainingDaysAfter + " jour(s) sur votre contrat !");
